...
def read_passwords_from_file(password_file):
    ...
    """
    Lê senhas de um arquivo de texto, uma por linha.
    """
    try:
...
#         password = get_archive_password(file_name)
...
#             with rarfile.RarFile(file_path, pwd=password) as rf:
...
def extract_compressed_file(file_path, temp_dir, original_data, passwords):
    """
    Extrai arquivos compactados recursivamente e processa cada arquivo.
    ...
    Args:
        ...
        temp_dir: Diretório temporário para extração
        original_data: Dados do arquivo compactado original
        passwords: Lista de senhas para tentar abrir arquivos protegidos
    """
    try:
        file_name = os.path.basename(file_path)
        ...
        for root, _, files in os.walk(extract_dir):
            for file in files:
                ...
                if file.endswith(('.zip', '.rar', '.7z')):
                    extract_compressed_file(
                        current_file, temp_dir, original_data, passwords)
                else:
                    # Processa o arquivo para OCR
                    content, success, error = extract_text_by_extension(
                        current_file, original_data=original_data, passwords=passwords)
...
def process_file_by_extension(file_path, extension, original_data=None, passwords=None):
    ...
    try:
        if extension in ['zip', 'rar', '7z']:
            with tempfile.TemporaryDirectory() as temp_dir:
                extract_compressed_file(
                    file_path, temp_dir, original_data or {}, passwords)
